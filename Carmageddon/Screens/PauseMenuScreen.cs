...
namespace Carmageddon.Screens
{
    class PauseMenuScreen : BaseMenuScreen
    {
        ...
        public PauseMenuScreen(PlayGameScreen parent)
            : base(parent)
        {
            _inAnimation = new FliPlayer(new FliFile(GameVars.BasePath + "data\\anim\\MAINCOME.fli"));
            _inAnimation.Play(false, 0);

            _outAnimation = new FliPlayer(new FliFile(GameVars.BasePath + "data\\anim\\MAINAWAY.fli"));

            _options.Add(
               new TextureMenuOption(BaseHUDItem.ScaleRect(0.202f, 0.185f, 0.65f, 0.045f),
                   new FliFile(GameVars.BasePath + "data\\anim\\MAINCNGL.fli").Frames[0])
           );
            _options.Add(
                new TextureMenuOption(BaseHUDItem.ScaleRect(0.172f, 0.356f, 0.715f, 0.045f),
                    new FliFile(GameVars.BasePath + "data\\anim\\MAINARGL.fli").Frames[0])
            );

            _options.Add(
                new TextureMenuOption(BaseHUDItem.ScaleRect(0.201f, 0.778f, 0.725f, 0.045f),
                    new FliFile(GameVars.BasePath + "data\\anim\\MAINQTGL.fli").Frames[0])
            );
        }
